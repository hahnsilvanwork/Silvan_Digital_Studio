...
OUT = ROOT / 'artifacts/nfc-hero/revision-02-composite'
...
def screen(kind, scroll=0):
    ...
    if kind in ('idle','nfc'):
        # A designed lockscreen; no invented URL or user account.
        tx(78,340,'9:41',88,False,'#777a76')
        if kind=='nfc':
            d.rounded_rectangle((15,80,375,232),radius=23,fill='white',outline='#d2d3cd',width=2)
            tx(32,95,'NFC-Tag erkannt',26,True)
            tx(32,139,'Menü öffnen',31,True)
            tx(32,184,'Zum Öffnen antippen',21,color='#555c55')
    elif kind=='menu':
...
FPS=30
writer=imageio_ffmpeg.write_frames(str(OUT/'nfc-hero-revision-02.mp4'),(W,H),fps=FPS,codec='libx264',pix_fmt_in='rgb24',pix_fmt_out='yuv420p',output_params=['-crf','18','-movflags','+faststart'])
writer.send(None)
...
for i in range(450):
    ...
    st=t*2 if t<4 else (8+(t-4)*5/8.5 if t<12.5 else 13+(t-12.5)*2/2.5)
    ...
    f,tracked=add_screen(f,pixels);tracking.append(tracked)
    f=add_card(f,kind)
...
print(OUT/'nfc-hero-revision-02.mp4')
