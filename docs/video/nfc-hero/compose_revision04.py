...
FPS=30
writer=imageio_ffmpeg.write_frames(str(OUT/'nfc-hero-revision-04.mp4'),(W,H),fps=FPS,codec='libx264',pix_fmt_in='rgb24',pix_fmt_out='yuv420p',output_params=['-crf','18','-movflags','+faststart'])
writer.send(None)
review_times=[0,.8,.95,1,1.4,2.4,2.8,3,3.2,3.4,3.6,3.8,4,4.4,5.6,6.4,7.2,8,8.8,10.5,12.5,14,14.9]
...
for i in range(450):
    ...
    tap_layer=None;tap_alpha=None
    if .75<=t<4:
        moving,_=stabilize(frames[min(round(st*24),len(frames)-1)].copy(),1)
        plate=frames[ANCHOR_INDEX].copy()
        # A fixed full plate avoids warping the backdrop, phone edges or holding hand.
        blend=np.clip((t-.75)/.2,0,1);blend=blend*blend*(3-2*blend)
        f=np.uint8(f*(1-blend)+plate*blend)
        quad=np.float32(quad*(1-blend)+ANCHOR*blend)
        if t>=2.6:
            b,g,r=cv2.split(moving.astype(np.int16))
            pb,pg,pr=cv2.split(plate.astype(np.int16))
            skin=(r-b>35)&(r>100)&(r-g>10)
            held=((pr-pb>35)&(pr>100)&(pr-pg>10)).astype('uint8')
            held=cv2.dilate(held,np.ones((9,9),np.uint8))>0
            held &= (ys>220)&(xs<1170)
            mask=(skin&~held&(xs>940)&(ys>105)&(ys<410)).astype('uint8')*255
            mask=cv2.morphologyEx(mask,cv2.MORPH_CLOSE,np.ones((5,5),np.uint8))
            # The tapping hand enters from the right edge; seal pale highlight gaps there.
            for row in range(106,410):
                occupied=np.flatnonzero(mask[row,1190:])
                if len(occupied):mask[row,1190+occupied[0]:]=255
            contours,_=cv2.findContours(mask,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
            mask[:]=0
            for c in contours:
                if cv2.contourArea(c)>250:cv2.drawContours(mask,[c],-1,255,cv2.FILLED)
            tap_alpha=cv2.GaussianBlur(mask,(3,3),.6).astype(np.float32)/255
            tap_layer=moving
    kind='menu'
...
print(OUT/'nfc-hero-revision-04.mp4')
